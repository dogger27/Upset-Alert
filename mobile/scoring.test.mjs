...

{
  ...
  // whatever the rounds say — until the final is played, when the closest
  // guess on the champion's aces, then on minutes, separates them.
  const e = [
  ...
  const played = [
    ...
    { total: 10, tie_aces_diff: 1, tie_minutes_diff: 5 },
    { total: 10 },                                        // never answered: last among the level
  ]
  assert.equal(sameStanding(played[1], played[2]), true)
  assert.equal(sameStanding(played[0], played[1]), false)
