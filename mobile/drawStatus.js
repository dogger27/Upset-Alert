/*
 * PORTED VERBATIM from frontend/src/utils/drawStatus.js.
 *
 * Copied rather than reimplemented on purpose. Which bucket a draw belongs in
 * is a product rule with real edge cases — cohort clustering, "Last Week" being
 * exactly one cohort, a draw staying Active until Pacific midnight — and two
 * implementations of it would disagree the first time either was touched. The
 * phone and the website must put a draw in the same place.
 *
 * It is plain JS with no DOM, so it runs here unchanged. If this file and the
 * web one ever differ, the web one is right; that is where the rules live.
 */
