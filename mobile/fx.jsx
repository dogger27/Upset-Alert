...
const NEON = ['#39ff14', '#00e5ff', '#b026ff', '#ff2d95', '#ffd400', '#39ff14']
const SHAKE = [[0.85, 0], [1.18, 7], [1.32, -7], [1.40, 6], [1.34, -6], [1.26, 3], [1.20, -2], [1.15, 0]]

/* The shake, as a transform for whoever hosts the chip. Separate from the
   ring because the chip is itself rotated -90°: a ring drawn INSIDE it turns
   with it, while the shake has to happen OUTSIDE it, around the whole
   rotated pill. */
export function useStandoutShake(on) {
  const reduce = useReduceMotion()
  const scale = useRef(new Animated.Value(on ? 1.15 : 1)).current
  const rot = useRef(new Animated.Value(0)).current
  useEffect(() => {
    if (!on) { scale.setValue(1); rot.setValue(0); return }
    scale.setValue(0.85); rot.setValue(0)
    const step = 1600 / SHAKE.length
    const shake = reduce
      ? Animated.timing(scale, { toValue: 1.15, duration: 0, useNativeDriver: true })
      : Animated.sequence(SHAKE.map(([sc, deg]) => Animated.parallel([
          Animated.timing(scale, { toValue: sc, duration: step, easing: EASE, useNativeDriver: true }),
          Animated.timing(rot, { toValue: deg, duration: step, easing: EASE, useNativeDriver: true }),
        ])))
    shake.start()
    return () => shake.stop()
  }, [on, reduce, scale, rot])
  const rotate = rot.interpolate({ inputRange: [-10, 10], outputRange: ['-10deg', '10deg'] })
  return on ? [{ scale }, { rotate }] : []
}

/* The neon ring: a border that walks the spectrum four times, then holds.
   The spin repaints every frame and is an arrival, not a permanent bill. */
export function NeonRing({ on, radius = 4 }) {
  ...
  useEffect(() => {
    ...
    const spin = Animated.loop(
      Animated.timing(hue, { toValue: 1, duration: 2400, easing: Easing.linear, useNativeDriver: false }),
      { iterations: 4 })
    spin.start()
    return () => spin.stop()
  }, [on, hue])
  if (!on) return null
  const borderColor = hue.interpolate({
    inputRange: NEON.map((_, i) => i / (NEON.length - 1)), outputRange: NEON,
  })
  return (
    <Animated.View pointerEvents="none"
                   style={[s.neon, { borderRadius: radius + 2, borderColor, shadowColor: borderColor }]} />
  )
}
