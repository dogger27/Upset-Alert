...
assert.equal(widthAt(LEVELS[0], 9, false), 34)
assert.equal(widthAt(LEVELS[0], 40, false), 56)
assert.equal(widthAt(LEVELS[0], 9, true), 40)
assert.equal(widthAt(LEVELS[3], 9, false), 15)             // no minimum at the tightest
...
assert.equal(r.total, 5 * 34 + 40 + 5 * 6)

// Nine days: the roomy level is 354 wide, so the strip steps down one.
r = pickLevel(digits(9), 4, 348)
assert.equal(totalAt(LEVELS[0], digits(9), 4), 8 * 34 + 40 + 8 * 6)
assert.equal(r.index, 1); assert.equal(r.fits, true)

// Twenty days fit only at the tightest — and DO fit, so no scrolling.
