...
# The tours' artwork lives in art/, outside the app: the app draws none of it
# any more (its tier stamps are set in Kanit) and Metro has no reason to crawl
# 1.3 MB of print-resolution downloads. The one output a client still needs —
# the US Open's flame — goes to the app's own badge/ directory below.
ART = Path(__file__).parent.parent / 'art' / 'logos'
APP_BADGE = Path(__file__).parent.parent / 'mobile' / 'assets' / 'logos' / 'badge'
...
for out_name, src_name in CRESTS.items():
    ...
    APP_BADGE.mkdir(parents=True, exist_ok=True)
    save_small(mark, APP_BADGE / out_name)
...
(Path(__file__).parent.parent / 'mobile' / 'tierStampAspect.js').write_text(
